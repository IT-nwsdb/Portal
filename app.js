...
function handleLogin(event) {
  ...
  if (!email || !password) {
    if (typeof showToast === 'function') showToast('Please enter both email and password', 'danger');
    return false;
  }
  ...
  setTimeout(() => { // simulate auth latency
    if (typeof DEMO_CREDENTIALS !== 'undefined' && email === DEMO_CREDENTIALS.email && password === DEMO_CREDENTIALS.password) {
      ...
      if (typeof showToast === 'function') showToast('Login successful! Welcome to NWSDB Portal.', 'success');
      ...
      if (typeof showToast === 'function') showToast('Invalid email or password. Use: nwsbrsccit@gmail.com / 123', 'danger');
    }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.getElementById('openAdminBtnPublic')?.addEventListener('click', () => {
  ...
  document.getElementById('openAdminBtnApp')?.addEventListener('click', () => {
    if (typeof window.showAdminPrompt === 'function') window.showAdminPrompt();
    else document.getElementById('loginModal') && bootstrap?.Modal.getOrCreateInstance(document.getElementById('loginModal')).show();
  });
});
